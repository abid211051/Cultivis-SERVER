...
map.post("/createpoly", async (c) => {
  try {
    ...
    const result = await db.insert(fieldPoly).values({ ...reqobj, userId: reqobj.userId }).returning({ id: fieldPoly.id, polygon: fieldPoly.polygon });
...
map.get("/getpoly/:userId", async (c) => {
  try {
    ...
    const db = connectdb(c);
    const field = await db.select({ id: fieldPoly.id, polygon: fieldPoly.polygon }).from(fieldPoly).where(eq(fieldPoly.userId, userId));
    if (field.length > 0) {
      return c.json(field[0]);
    }
...
// .wrangler/tmp/bundle-AAVAYU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-AAVAYU/middleware-loader.entry.ts
