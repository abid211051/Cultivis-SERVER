...
var weather = new Hono2();
weather.get("/currentweather", async (c) => {
  try {
    return c.json(mock_default.currentWeatherData);
  } catch (error) {
    console.log(error);
  }
});
weather.get("/hourlyforecast", async (c) => {
  try {
    ...
    let flag = false;
    for (const element of mock_default?.hourlyWeatherData?.list) {
      const dt_hour = (/* @__PURE__ */ new Date(`${element.dt_txt}`)).getHours();
...
weather.get("/dailyforecast", async (c) => {
  try {
    const currentday = (/* @__PURE__ */ new Date()).toDateString();
    let fivedaydata = [];
    let flag = false;
    for (const element of mock_default?.dailyWeatherData?.list) {
      const dataInData = new Date(parseInt(element.dt) * 1e3).toDateString();
      ...
      if (flag && fivedaydata.length < 6) {
        fivedaydata.push({
          ...
          icon: element?.weather?.[0].icon,
          precipitation: Math.round(parseFloat(element?.pop) * 100),
          temperature: Math.round(parseFloat(element?.temp.max)) + "/" + Math.round(parseFloat(element?.temp.min))
        });
...
// src/db/schema/mapSchema.js
var fieldPoly = pgTable("fields", {
...
var cropInfo = pgTable("cropInfo", {
  id: uuid().defaultRandom().primaryKey(),
  fieldId: uuid().notNull().references(() => fieldPoly.id, { onDelete: "cascade" }),
...
map.post("/createpoly", async (c) => {
  try {
    ...
    const result = await db.insert(fieldPoly).values({
...
map.get("/getpoly/:userId", async (c) => {
  try {
    ...
    }).from(fieldPoly).leftJoin(cropInfo, eq(fieldPoly.id, cropInfo.fieldId)).where(eq(fieldPoly.userId, userId));
    if (field.length > 0) {
      return c.json(field);
    }
...
map.put("/editpoly/:id", async (c) => {
  try {
    ...
    const db = connectdb(c);
    const updatedField = await db.update(fieldPoly).set(body).where(eq(fieldPoly.id, id)).returning({ id: fieldPoly.id, polygon: fieldPoly.polygon });
...
map.delete("/deletepoly/:id", async (c) => {
  try {
    const { id } = c.req.param();
    const db = connectdb(c);
    const deleted = await db.delete(fieldPoly).where(eq(fieldPoly.id, id)).returning({ id: fieldPoly.id });
...
map.post("/setcropinfo", async (c) => {
  try {
    const body = await c.req.json();
    const db = connectdb(c);
    const cropdata = await db.insert(cropInfo).values({
      fieldId: body.fieldId,
      cropName: body.cropname,
      maturityDay: body.maturity,
      cropType: body.croptype,
      sowing: body.sowing,
      harvesting: body.harvesting,
...
app.route("/weather", weather_default);
app.route("/map", map_default);
var src_default = app;

// C:/Users/apran/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/apran/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-PNM6Qt/middleware-insertion-facade.js
...
// C:/Users/apran/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-PNM6Qt/middleware-loader.entry.ts
