...
map.delete("/deletepoly/:id", async (c) => {
  try {
    const { id } = c.req.param();
    const db = connectdb(c);
    await db.delete(fieldPoly).where(eq(fieldPoly.id, id));
...
// .wrangler/tmp/bundle-PKUIn6/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-PKUIn6/middleware-loader.entry.ts
